...
export const PayoutModal = ({ user, onClose, onPayout }: PayoutModalProps) => {
  const [amount, setAmount] = useState<number | ''>(user.stats.balance > 0 ? user.stats.balance : '');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (Number(amount) > 0) {
      onPayout(Number(amount));
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="bg-amber-100 p-4 border-b border-amber-200">
          <h3 className="text-lg font-bold text-amber-900 flex items-center">
            <HandCoins className="mr-2" /> Выплата зарплаты / Аванс
          </h3>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="bg-stone-50 p-4 rounded-xl border border-stone-200 flex justify-between items-center">
            <span className="text-stone-600">{user.stats.balance < 0 ? 'Текущий аванс:' : 'Текущий долг:'}</span>
            <span className={`font-bold text-xl ${user.stats.balance < 0 ? 'text-emerald-600' : 'text-stone-800'}`}>
              {user.stats.balance < 0 ? Math.abs(user.stats.balance) : user.stats.balance} ₽
            </span>
          </div>
